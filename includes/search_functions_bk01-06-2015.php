...
class search_functions 
{
	...
	function WS_GET_SEARCH_RESULT($keywords, $limit='5')
	{
		...
		$sql="SELECT * FROM home_slider WHERE H_IMAGE_NAME like '%".$keywords."%' OR H_IMG_DESC like '%".$keywords."%' LIMIT $limit" ;
		...
		$sql="SELECT * FROM portfolio_galleries WHERE GAL_NAME like '%".$keywords."%' OR GAL_SUB_TITLE like '%".$keywords."%' LIMIT $limit" ;
	...
	function WS_GET_SEARCH_COUNT_RESULT($keywords)
	{
		...
		$sql="SELECT COUNT(*) AS count FROM home_slider WHERE H_IMAGE_NAME like '%".$_POST['search_txt']."%' OR H_IMG_DESC like '%".$_POST['search_txt']."%'" ;
		...
		$sql="SELECT COUNT(*) AS count FROM portfolio_galleries WHERE GAL_NAME like '%".$_POST['search_txt']."%' OR GAL_SUB_TITLE like '%".$_POST['search_txt']."%'" ;
	...
	function WS_GET_SEARCH_RESULT1($keywords, $start, $perpage)
	{
		global $row,$_POST; 
		$data = $_POST;
        $row=new DB;
		...
		$sql="SELECT * FROM portfolio_galleries WHERE GAL_NAME like '%".$keywords."%' OR GAL_SUB_TITLE like '%".$keywords."%' LIMIT $start, $perpage" ;
			  ...
			  {	
					...
					foreach(array_keys($client_result) as $key){
 				   	//echo gettype($key)."\n";
							...
							{
								//array_push($record,$key,$client_result[$key]);
								$record[$key] = strip_tags(stripslashes(stripslashes(stripslashes($client_result[$key]))));
							}
					}
