...
class search_functions 
{
	...
	function WS_GET_SEARCH_COUNT_RESULT($keywords)
	{
		...
		$sql="SELECT COUNT(*) AS count FROM home_slider WHERE H_IMAGE_NAME like '%".$keywords."%' OR H_IMG_DESC like '%".$keywords."%' OR H_TITLE like '%".$keywords."%'" ;
	...
	function WS_GET_IMAGE_NAME($gal_id)
	{
		...
		$row=new DB;
		$image_name=array();
		$sql="SELECT H_TITLE FROM home_slider where GAL_ID='".$gal_id."' and HS_ID=2 LIMIT 1,1";
		$db_query=$row->query($sql);
		while($client_result=$row->next_record())
		{
			$record = array();
