...
{ 
	...
	include("../includes/herluf.php");	// logic writen in this class
	$HI = new herluf;
	$directory_name='Herluf-intern';
	$all_directory = $HI -> get_herluf_derectory_data($directory_name);
	if($all_directory[0]['ulb_cnt']!=0)
	{ // if start $all_directory[0]['ulb_cnt'] start 
		for($t=0;$t<count($all_directory);$t++)
		{  // for loop start 
		$str_bro = str_replace("photos","thumbnails",$all_directory[$t]['image_path']);
		$file_path="../".$str_bro;
		
			if(!file_exists($file_path))
			{ // if !file exists is start 
				$db_query="	UPDATE 	image_data
				SET 	album_Id= 1000000
				WHERE 	image_album= '".$directory_name."'
				and		image_id = '".$all_directory[$t]['image_id']."'";
				$db_query1=$row->query($db_query);
			} // if !file exists is End 
		} // for loop end 
	} // if start $all_directory[0]['ulb_cnt'] End  
	
	$USR_GAL = $HI->get_gallery_directory($directory_name);
	print_r($USR_GAL);
	exit;
