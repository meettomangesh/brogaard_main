...
// Arafin:Retreive folder permission
if($_SESSION['folderaccess']==1){
	$FileAccessCondition = " AND image_directory !='HD' ";
}elseif($_SESSION['folderaccess']==2){
	$FileAccessCondition = " AND image_directory !='galleries' ";
}

$search_word = ( !empty($_GET['s']) ? strtolower($_GET['s']) : '' );
//str_replace("world","Peter","Hello world!");
$search_word = str_replace("Ø","¯",$search_word);
$search_word = str_replace("ø","¿",$search_word);
$search_word = str_replace("å","aa",$search_word);
//$search_word = str_replace("Ø","¯",$search_word);
echo $search_word;
// Start Check for Danish letter
//echo $_GET['s'];echo "<br />";
//echo $url = urlencode($_GET['s']);echo "<br />";
/*
...
// End Check for Danish letter
*/

// #################
// IF search exist
if ( empty($_SESSION['hdaccess']) && $_SESSION['maingallery']==PAID_HD_URL  ) {
	
}
	if ( empty($_SESSION['hdaccess']) && $_SESSION['maingallery']==PAID_HD_URL ) {
	}else{
		if( !empty($search_word) && !empty( $_GET['st'] ) )
			$HDQuery = " AND image_directory ='HD' ";
	}
...
{  
	if ( empty($_SESSION['galleries']) ) { 
		$herePath = str_replace(CMS_URL, '', $_SESSION['redirect']); } 
	else { $herePath = $_SESSION['galleries']; }
// image_data
	$sql = "SELECT left(image_path, length(image_path)-locate('/',reverse(image_path))) as pathname, group_concat(image_id) FROM ".TBL_IMAGE_DATA." WHERE (image_description LIKE '%".clean_mysql($search_word)."%' OR image_keywords LIKE '%".clean_mysql($search_word)."%' OR image_name LIKE '%".clean_mysql($search_word)."%') $HDQuery $FileAccessCondition AND image_path RLIKE '".$herePath."' GROUP BY left(image_path, length(image_path)-locate('/',reverse(image_path))) ORDER BY image_id ASC";
...
$found = array_sum($totalIDx); // TOTAL valid images found
	...
	if( !empty( $_GET['st'] ) ){
	echo '<p class="desc-paragraph">Vi har fundet <u>'.( !empty($found)?$found:'0' ).'</u> billede(r) fra din søgning på <u>'.( !empty($search_word)?$_GET['s']:'' ).'</u> i <img src="'.URL_USER.'pages-resources/images/icon_gallery_HD.png" class="img-class" /></p>'; 
	 }else{
	 	echo '<p class="desc-paragraph">Vi har fundet <u>'.( !empty($found)?$found:'0' ).'</u> billede(r) fra din søgning på <u>'.( !empty($search_word)?$_GET['s']:'' ).'</u></p>'; 
	 }
	...
	if ( empty($_SESSION['hdaccess']) && $_SESSION['maingallery']==PAID_HD_URL ) {
		...
		//echo '<p class="desc-paragraph"><a href="http://www.brogaard.com/shell/search.php?s="';
		//echo 'BRO_20120928_IDA_Carl_A_Dahl_358';
		//echo '&st=h>Se kun HD-billeder</a> HD-graphic</p>';//&& $_SESSION['maingallery']!=PAID_HD_URL 
	?>
    <?php
		// Check ho many HD pics are for this search
		$result1 = mysql_query("SELECT left(image_path, length(image_path)-locate('/',reverse(image_path))) as pathname, group_concat(image_id) FROM ".TBL_IMAGE_DATA." WHERE (image_description LIKE '%".clean_mysql($search_word)."%' OR image_keywords LIKE '%".clean_mysql($search_word)."%' OR image_name LIKE '%".clean_mysql($search_word)."%') AND image_directory ='HD' AND image_path RLIKE '".$herePath."' GROUP BY left(image_path, length(image_path)-locate('/',reverse(image_path))) ORDER BY image_id ASC");
		$HDPhotoExist = mysql_num_rows($result1);
		//If no HD, then don't show the next options
		if($HDPhotoExist != 0){
			//If no request to see HD, then show this
			if( empty( $_GET['st'] ) ){
				if ( ($_SESSION['folderaccess'] != 2) ){
		?>
		<p class="desc-paragraph"><a href="http://www.brogaard.com/shell/search3.php?s=<?php echo $_GET['s']; ?>&st=h"><img src="<?php echo URL_USER; ?>pages-resources/images/icon_gallery_HD.png" class="img-class-se-kun" /> Se kun HD-billeder</a></p> 
		<?php
				}
			}else{
		?>
		<p class="desc-paragraph"><a href="http://www.brogaard.com/shell/search3.php?s=<?php echo $_GET['s']; ?>"><img src="<?php echo URL_USER; ?>pages-resources/images/icon_gallery.png" height="18px" width="18px" /> Vis alle billeder der indeholder <?php echo $search_word; ?></a></p> 	
		<?php } ?>
		<?php
			}
		}
...
?>
		...
		<p class="desc-paragraph">Klik på <img src="<?php //echo URL_USER; ?>pages-resources/images/icon_gallery.png" /> for at gå til original-gallerierne.</p> -->
		<p class="desc-paragraph">&nbsp;</p>
		<p class="desc-paragraph">&nbsp;</p>
		<div class="desc-paragraph last-div" nowrap="nowrap"><span style="font-size:70%;">Bruger du denne funktion accepterer du betingelserne i <a href="<?php echo URL_USER; ?>terms.php" title="Terms of Use" target="_self"><u>Terms of Use</u></a>. Alle billeder &copy; Steen Brogaard</span></div>
		</section>
</div>
</div>
...
while ( $data = mysql_fetch_assoc($query) )
	{ 
	  ...
	  $path2exclude = PAID_HD_DIR.'/HD';
		...
		echo '<h2 style="'.$stylGallery.'"> '.$gallName.' ( '.$totalID.' ) </h2></div><div style="float:right; margin-top:14px;"></div><div style="clear:both;"></div>'."\n"; 
