...
function o_info($id,$action,$options=''){
  ...
  switch ($action){
    case 'new':
      $this->infoObject=$id;
      $date='D:'.date('Ymd');
      $this->objects[$id]=array('t'=>'info','info'=>array('Creator'=>'PDF Writer, http://www.brogaard.com/','CreationDate'=>$date));
      break;
    case 'Title':
    case 'Author':
    case 'Subject':
...
function stream($options=''){
  ...
  if ( isset($options['compress']) && $options['compress']==0){
	...
	header("Pragma: public");											// added for Force Download
	header("Expires: 0");												// added for Force Download
	header("Cache-Control: must-revalidate, post-check=0, pre-check=0");// added for Force Download
	header("Cache-Control: public");									// added for Force Download
	header("Content-Description: File Transfer");						// added for Force Download
	header("Content-type: application/force-download");					// MODS for Force Download
	header("Content-Transfer-Encoding: Binary");						// added for Force Download
  //header("Content-type: application/pdf");	
  header("Content-Length: ".strlen(ltrim($tmp)));
  $fileName = (isset($options['Content-Disposition'])?$options['Content-Disposition']:'brogaard.pdf');
  header("Content-Disposition: attachment; filename=".$fileName); // 'inline' changed to 'attachment'
