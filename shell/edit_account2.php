...
if ( $_SESSION['redirect']=='' ) {
	...
	$redirecttext = '<a href="'.$_SESSION['redirect'].'">'.$_SESSION['redirect'].'</a>';
}
...
echo '<span style="color:#111111; font-style:italic; float:left; padding-top:10px; padding-left:10px;">'.$welcome_message_add.'</span>';
?>
<img class="edit-acc-image" alt="" src="<?php echo URL_USER; ?>pages-resources/images/pic_konto.png" />
</div>
...
<li style="height:30px;"><u><?php echo $redirecttext.'</u> - '.lang('main_gallery'); ?></li>
<?php 
...
for($i = 0; $i < $howmany; ++$i){
	...
	//echo array_sum($total_thumbs_in_each_thumbsDir[$i]);
	...
	echo '<li style="height:30px;"><u><a href="'.$link_ac.'">'.$link_ac.'</a></u> ('.$totalThumbsALL.')</li>';
...
<?php } else { echo '<div style="height:400px;">&nbsp;</div>'; } ?>
...
<?php if ( !empty($_SESSION['galleries']) ) { 
  ...
  // LOOKUP Latest Uploaded Gallery for user WITHOUT Gallery access
    $lookOnRoyal = ( !empty($_SESSION['royalaccess']) ? '|clientarea/royal' : '' );
	$oneGalleryAccess = str_replace(CMS_URL, '', $_SESSION['redirect']);
